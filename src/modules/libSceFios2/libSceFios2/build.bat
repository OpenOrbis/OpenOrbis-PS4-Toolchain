...
for %%f in (*.cpp) do (
    clang++ -cc1 -triple x86_64-scei-ps4-elf -shared -munwind-tables -I"%OO_PS4_TOOLCHAIN%\include" -emit-obj -o %intdir%\%%~nf.o %%~nf.cpp
)
...
ld.lld -m elf_x86_64 -shared --script "%OO_PS4_TOOLCHAIN%\link_prx.x" --eh-frame-hdr -o "%outputElf%" "-L%OO_PS4_TOOLCHAIN%\lib" %libraries% --verbose "%OO_PS4_TOOLCHAIN%\lib\crtlib.o" %obj_files%
