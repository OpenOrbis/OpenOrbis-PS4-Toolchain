...
set PKG_TITLE="OpenOrbis Threading Sample"
set PKG_VERSION="1.02"
set PKG_ASSETS="assets"
set PKG_TITLE_ID="BREW00087"
set PKG_CONTENT_ID="IV0000-BREW00087_00-PTHREADDEX000000"

Rem Libraries to link in
set libraries=-lc -lkernel -lc++ -lSceVideoOut -lSceSysmodule -lSceFreeType
...
for %%f in (..\..\_common\*.c) do (
    clang -c -target x86_64-pc-freebsd-elf -funwind-tables -fPIC -I"%OO_PS4_TOOLCHAIN%\\include" -I"%OO_PS4_TOOLCHAIN%\\include\\c++\\v1" -fuse-init-array %extra_flags% -o %intdir%\%%~nf.o ..\..\_common\%%~nf.c
)

for %%f in (..\..\_common\*.cpp) do (
    clang++ -c -target x86_64-pc-freebsd-elf -funwind-tables -fPIC -I"%OO_PS4_TOOLCHAIN%\\include" -I"%OO_PS4_TOOLCHAIN%\\include\\c++\\v1" -fuse-init-array %extra_flags% -o %intdir%\%%~nf.o ..\..\_common\%%~nf.cpp
)
