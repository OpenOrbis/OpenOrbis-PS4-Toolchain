...
set PKG_TITLE_ID="BREW00071"
set PKG_CONTENT_ID="IV0000-BREW00071_00-PIGLETEXAMPLE000"

Rem Libraries to link in
set libraries=-lc -lkernel -lc++ -lSceShellCoreUtil -lSceSysmodule -lSceSystemService -lScePigletv2VSH -lScePrecompiledShaders
...
Rem set this variable to your llvm bin folder...
Rem NIK NOTE: I do not have LLVM tools in my PATH so I always edit build bats to point to it manually.
Rem just comment out this line and get rid of %clangPath%\ in the lines below:
set clangPath=D:\SDK\LLVM\bin

@mkdir %intdir%

Rem Compile object files for all the source files
for %%f in (*.cpp) do (
    %clangPath%\clang++ -cc1 -triple x86_64-pc-freebsd-elf -munwind-tables -I. -I"%OO_PS4_TOOLCHAIN%\include" -I%commonDir% -I"%OO_PS4_TOOLCHAIN%\include\c++\v1" -fuse-init-array %extra_flags% -debug-info-kind=limited -debugger-tuning=gdb -emit-obj -o %intdir%\%%~nf.o %%~nf.cpp
)
...
%OO_PS4_TOOLCHAIN%\bin\windows\create-eboot.exe -in "%outputElf%" --out "%outputOelf%" --paid 0x3800000000000011 -authinfo %sys_authinfo%
...
%OO_PS4_TOOLCHAIN%\bin\windows\PkgTool.Core.exe sfo_setentry sce_sys/param.sfo APP_TYPE --type Integer --maxsize 4 --value 0
